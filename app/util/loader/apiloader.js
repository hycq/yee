...
function normalizeHost(host, isWebsocket) {
  ...
  hosts = window.location.host + hosts
  const normalProtocol = window.location.protocol
  if(isWebsocket) {
...
export default {
  ...
  url(key, data = {}) {
    ...
    if(questionMarkIndex === -1) return requiredURL
    ...
    const realUrl = requiredURL.substring(0, questionMarkIndex) + realOptParams
